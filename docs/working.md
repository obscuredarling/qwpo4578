## Creating a New Project 

1. In Visual Studio Code, click ![RVBuilder icon](./images/RVBuilder_icon.png) to open the RVBuilder view and click **Home**. 
2. On the RVBuilder Home page, click **New Project**. 
3. On the **New Project** dialog, enter a project name, specify a programming language (C or C++), select a desired Andes RISC-V target and project type, and then click **Browse** to specify a project workspace location.

   **Note**

   - Andes RISC-V targets are defined by chip profiles. For more about chip profiles in the RVBuilder package, see [**Chip Profiles**](./components.md#chip-profiles).

4. Click **Create** to complete the project creation. 
![RVBuilder project creation](./images/project_create.png)



## Configuring Project Settings



## Building a Project 



## Debugging a Project



## Flash Programming 

