...
### `tasks.json`
For projects that use the Makefile automatically generated by RVBuilder (see [**Target Configuration**](./project_config.md#target-configuration) for a project's Makefile setting), the build task settings in the task configuration file are synchronized with the target configuration and build options specified in the project settings.

However, if the project uses a custom Makefile (that is, the option "Use RVBuilder-generated Makefile" is unchecked in a project's [**Target Configuration**](./project_config.md#target-configuration)), changing the project toolchain requires you to manually update the toolchain executable paths in the file. Otherwise, build errors may occur. 
