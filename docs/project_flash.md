...
For Andes RISC-V targets connected via AICE, Maverick, or GDB server, RVBuilder provides a graphical interface for programming application binaries into the target flash memory. 
...
## Flash Burn Configuration

The following describes the configuration options available in the **Flash Burner** UI for each programming approach in its corresonding tab. The RVBuilder package includes the flash burners and target applications required for the **Flash Burn** and **Target Burn** aproaches (see [**Flash Burner**](./using_rvbuilder.md#flash-burners)). To use a self-defined flash burner, go to the **Custom Burner** tab and specify the burner and its related arguments. 
...
| Host | Specifies the name of IP address of the remote host. Required only for Andes RISC-V targets connected via GDB. |
| Burner Port| Specifies a socket port for the burner to connect. Required only for Andes RISC-V targets connected via GDB.|
...
| Host | Specifies the name of IP address of the remote host. Required only for Andes RISC-V targets connected via GDB. |
| Telnet Port| Specifies a socket port for the burner to connect. Required only for Andes RISC-V targets connected via GDB. |
| Unlock | Unlocks the flash memory before burning. |
| Lock After Programming| Locks the flash memory after programming. |
| Verification | Verifies the result after programming.  |
| Erase All | Erases the entire flash segment to perform programming. |
...
| Misc Arguments | Specifies the arguments for the flash burner. |
