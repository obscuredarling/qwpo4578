...
### `tasks.json`
For projects that use the Makefile automatically generated by RVBuilder (that is, the option **Use RVBuilder-generated Makefile** is selected in a project's [**Makefile**](./project_config.md#makefile-section) section), the build task settings in the task configuration file are synchronized with what are configured in the project settings.

However, if the project uses a custom Makefile (i.e., the option **Use RVBuilder-generated Makefile** is unchecked in a project's [**Makefile**](./project_config.md#makefile-section) section), changing the project toolchain requires you to manually update the toolchain executable paths in the file. Otherwise, build errors may occur. 
...
- `setupCommands` specifies debugger commands that are executed automatically at the start of the debug session. You can add additional GDB run commands to this section.  
...
RVBuilder supports two approaches to perform in-system programming on Andes RISC-V ICE targets (i.e., AndeShape development platforms connecting to the debug host via AICE, Maverick, or GDB server) . 
...
### Demo Applications
The RVBuilder package provides two demo applications, `demo-plic-novector-V5` and `freertos-V5`, to verify the development environment setup and demonstrate the development workflow using RVBuilder in VS Code. 

#### Non-vectored PLIC Related Interrupt Demo `demo-plic-novector-V5`
This demo program demonstrates how to activate the following machine interrupts and interrupts related to machine interrupts and PLIC (Platform-Level Interrupt Controller) interrupts:
...
#### FreeRTOS Demo Applications `freertos-V5`
